          ...
          <div key={item.id} className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-all group">
             <div className="flex justify-between items-start">
                <div className="flex items-start space-x-3">
                    <div className={cn("p-3 rounded-xl", item.stock <= item.minStock ? "bg-red-50 text-red-600" : "bg-emerald-50 text-emerald-600")}>
                        <Package size={24} />
                    </div>
                    <div>
                        <h3 className="font-bold text-slate-900">{item.name}</h3>
                        <p className="text-sm font-medium text-slate-500">{formatCurrency(item.price)} / {item.unit}</p>
                    </div>
                </div>
             ...
             <div className="mt-4 pt-4 border-t border-gray-50 flex justify-between items-center">
                ...
                <div className={cn("flex items-center space-x-1 font-bold", item.stock <= item.minStock ? "text-red-600" : "text-emerald-700")}>
