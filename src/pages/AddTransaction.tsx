...
export function AddTransaction() {
    ...
    const [paymentMode, setPaymentMode] = useState<'Cash' | 'Bank' | 'Online' | 'Credit'>('Cash');
    const [receiptImage, setReceiptImage] = useState<Blob | string | undefined>();
    ...
    const categories = useLiveQuery(() =>
        currentBusiness?.id ? db.categories.where('businessId').equals(currentBusiness.id).toArray() : []
        , [currentBusiness?.id]);

    const parties = useLiveQuery(() =>
        currentBusiness?.id ? db.parties.where('businessId').equals(currentBusiness.id).toArray() : []
        , [currentBusiness?.id]);

    const items = useLiveQuery(() =>
        currentBusiness?.id ? db.items.where('businessId').equals(currentBusiness.id).toArray() : []
        , [currentBusiness?.id]);
    ...
    const handleSave = async () => {
        if (!amount || !currentBusiness?.id) return;
    ...
    const addCategory = async () => {
        ...
        if (newCat) {
            await db.categories.add({ name: newCat, type: type, businessId: currentBusiness.id });
            setCategory(newCat);
        }
    };
                            ...
                            {(['Cash', 'Bank', 'Online'] as const).map((mode) => (
                                <button
                                    ...
                                    className={cn(
                                        ...
                                        paymentMode === mode && !isCredit ? "bg-white text-slate-800 shadow-sm" : "text-slate-500 hover:bg-white/50"
                                    )}
                                >
                                    {mode}
                                </button>
